...
async def weather(
    ...
    try:
        response = openai.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",  # 系統角色，設定AI的身份和任務ㄒ
                    "content": "你是一個專業的氣象分析師，請根據使用者提供的天氣預報資料，分析未來五天的天氣趨勢，並給出簡明扼要的總結。",
                },
                {
                    "role": "user",  # 使用者角色，提供AI需要分析的資料
                    "content": f"以下是未來幾天的天氣預報資料：{raw_forecast['list']}，請根據這些數據提供詳細的天氣分析和建議:\n{raw_forecast}",
                },
            ],
            temperature=0.2,  # 溫度越低，回應越穩定；溫度越高，回應越多樣化
        )
        analysis = response.choices[0].message.content
        await interaction.followup.send(f"{city}市的天氣AI分析結果：\n{analysis}")
    except Exception as e:
        await interaction.followup.send("分析天氣資料時發生錯誤，請稍後再試。")
        print(f"AI分析錯誤: {e}")
